...
const RECENT_URL = "https://mangapill.com/mangas/new";
...
function isCacheValid() {
  ...
  const { cacheMinutes, lastCached } = config.mangapill.caching.recent;
...
function updateLastCached() {
  const config = getConfig();
  config.mangapill.caching.recent.lastCached = new Date().toISOString();
  saveConfig(config);
}
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    console.error("[Mangapill Recent] Error:", error.message);
    res.status(500).json({
      error: "Failed to fetch recent mangas",
